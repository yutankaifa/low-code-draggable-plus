...
export interface UseDraggableOptions<T> extends Options {
  ...
  /**
   * A hook function that allows you to modify the cloned data before it's added to the list
   * @param {Fn} fn - The function that contains the cloned element data
   * @returns {Fn} - The modified function with processed data
   */
  modifyDataBeforeAdd?: (data: any) => any
}
...
export function useDraggable<T>(...args: any[]): UseDraggableReturn {
  ...
  function onAdd(evt: DraggableEvent) {
    ...
    const modifyDataBeforeAdd = unref(options)?.modifyDataBeforeAdd
    const newData = modifyDataBeforeAdd ? modifyDataBeforeAdd(element) : element
    removeNode(evt.item)
    if (isRef<any[]>(list)) {
      const newList = [...unref(list)]
      list.value = insertElement(newList, evt.newDraggableIndex!, newData)
      return
    }
    insertElement(unref(list), evt.newDraggableIndex!, newData)
  }
  ...
  const start = (target?: HTMLElement) => {
    ...
    instance = new Sortable(target as HTMLElement, options)
  }
